<?php
class ModelExtensionModuleSoAdvancedSearch extends Model {
	...
	public function install() {
		...
		");
        $this->db->query("
			CREATE TABLE IF NOT EXISTS `".DB_PREFIX."so_fluel` (
			  	`fluel_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
			  	`model_id` int(11) NOT NULL,
			  	`make_id` int(11) NOT NULL,
			  	`fluel_name` varchar(255) NOT NULL,
			  	PRIMARY KEY (`fluel_id`)
			) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8;
		");
        $this->db->query("
			CREATE TABLE IF NOT EXISTS `".DB_PREFIX."so_body` (
			  	`body_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
			  	`model_id` int(11) NOT NULL,
			  	`make_id` int(11) NOT NULL,
		...
		$this->model_setting_module->deleteModulesByCode('so_advanced_search');
		$module_data = array(
			'name' 		=> 'So Advanced Search',
			'status'	=> 1
		);
		$this->model_setting_module->addModule('so_advanced_search', $module_data);
	}
	...
	public function uninstall() {
        ...
        $this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."so_fluel`");
        $this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."so_body`");
