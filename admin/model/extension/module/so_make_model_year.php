<?php
class ModelExtensionModuleSoMakeModelYear extends Model {
    ...
    public function getFluels($data = array()) {
        $sql = "SELECT * FROM `".DB_PREFIX."so_fluel`";
        if (isset($data['model_id']) && !empty($data['model_id'])) {
            $sql .= " WHERE model_id = '".(int)$data['model_id']."'";
        }
        $sql .= " ORDER BY `fluel_name` ASC";

        $query = $this->db->query($sql);

        return $query->rows;
    }
    public function getBodies($data = array()) {
        $sql = "SELECT * FROM `".DB_PREFIX."so_body`";
        if (isset($data['model_id']) && !empty($data['model_id'])) {
            $sql .= " WHERE model_id = '".(int)$data['model_id']."'";
        }
        $sql .= " ORDER BY `body_name` ASC";
    ...
    public function getBodyById($body_id) {
        $sql = "SELECT * FROM `".DB_PREFIX."so_body` WHERE body_id = '".(int)$body_id."'";
        $query = $this->db->query($sql);

        return $query->row;
    }
    public function getFluel($fluel_id) {
        $sql = "SELECT * FROM `".DB_PREFIX."so_fluel` WHERE fluel_id = '".(int)$fluel_id."'";
        $query = $this->db->query($sql);
    ...
    public function addBody($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "so_body SET body_name = '" . $this->db->escape($data['body_name']) . "', model_id = '".(int)$data['model_id']."', make_id = '".(int)$data['make_id']."'");

        $body_id = $this->db->getLastId();

        return $body_id;
    }
    public function addFluel($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "so_fluel SET fluel_name = '" . $this->db->escape($data['fluel_name']) . "', model_id = '".(int)$data['model_id']."', make_id = '".(int)$data['make_id']."'");

        $fluel_id = $this->db->getLastId();
    ...
    public function editBody($body_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "so_body SET body_name = '" . $this->db->escape($data['body_name']) . "', model_id = '".(int)$data['model_id']."', make_id = '".(int)$data['make_id']."' WHERE body_id = '".(int)$body_id."'");
    }
    public function editFluel($fluel_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "so_fluel SET fluel_name = '" . $this->db->escape($data['fluel_name']) . "', model_id = '".(int)$data['model_id']."', make_id = '".(int)$data['make_id']."' WHERE fluel_id = '".(int)$fluel_id."'");
    }
    ...
    public function deleteBody($body_id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "so_body WHERE body_id = '" . (int)$body_id . "'");
    }
    public function deleteFluel($fluel_id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "so_fluel WHERE fluel_id = '" . (int)$fluel_id . "'");
    }
