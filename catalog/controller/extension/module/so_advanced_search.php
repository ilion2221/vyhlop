...
class ControllerExtensionModuleSoAdvancedSearch extends Controller {
	public function index($setting) {
		...
		$this->document->addStyle('catalog/view/javascript/so_advanced_search/css/style.css');
		...
		$data['makes'] = $this->model_extension_module_so_advanced_search->getMakes();
		$data['models'] = $this->model_extension_module_so_advanced_search->getModels();
		$data['engines'] = $this->model_extension_module_so_advanced_search->getEngines();
		$data['years'] = $this->model_extension_module_so_advanced_search->getYears();

		$data['module'] = $module++;

		return $this->load->view('extension/module/so_advanced_search/default', $data);
	}
	...
	public function getModel() {
		...
		if (isset($this->request->post['make_id']) && !empty($this->request->post['make_id'])) {
			$this->load->model('extension/module/so_advanced_search');
			$models = $this->model_extension_module_so_advanced_search->getModels(array('make_id'=>$this->request->post['make_id']));
			if (!empty($models)) {
	...
	public function getEngine() {
		...
		if (isset($this->request->post['model_id']) && !empty($this->request->post['model_id'])) {
			$this->load->model('extension/module/so_advanced_search');
			$engines = $this->model_extension_module_so_advanced_search->getEngines(array('model_id'=>$this->request->post['model_id']));
			if (!empty($engines)) {
	...
	public function getYear() {
		...
		if (isset($this->request->post['engine_id']) && !empty($this->request->post['engine_id'])) {
			$this->load->model('extension/module/so_advanced_search');
			$years = $this->model_extension_module_so_advanced_search->getYears(array('engine_id'=>$this->request->post['engine_id']));
			if (!empty($years)) {
