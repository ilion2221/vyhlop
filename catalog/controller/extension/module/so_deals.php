		...
		if($setting['display_feature']){
			...
			if(count($setting['product_feature']) > 0){
				...
				foreach($products_deals as $product)
				{
	        		...
	        		foreach ($product_image as $image_gallery) {
	        			$image_galleries[] = array(
	        				'gallery_image' => $this->model_tool_image->resize($image_gallery['image'], $setting['width'], $setting['height']),
	        				'gallery_thumb' => $this->model_tool_image->resize($image_gallery['image'], 100, 100),
	        			);
	        		}
